...
const MovieDetails = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      setMovieDetails(await getMovieDetails({ movieId: id, signal: controller.signal }));
    })();
    return () => {
      controller.abort();
    };
  ...
  return (
    <>
      <Row className="bg-body-tertiary rounded-4 my-4 mx-0">
        <Col xs="12" md="5">
          <img src={imgPath} alt={movieDetails.original_title} title={movieDetails.title} className="poster my-2 rounded-3" />
        </Col>

        <Col xs="12" md="7" className="py-2 px-4 fs-5">
          <div className="d-flex flex-column align-items-center align-items-md-start justify-content-center h-100 ">
      ...
      <Row className="bg-body-tertiary rounded-4 my-4 mx-0 p-2">
        <Col sm="12" className="d-flex gap-2 justify-content-center ">
          <Link to="/"><Button className="bg-custom-brown border-danger" variant="danger">عودة للرئيسية</Button></Link>
